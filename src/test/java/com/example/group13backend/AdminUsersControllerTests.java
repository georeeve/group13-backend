...
public class AdminUsersControllerTests {
    ...
    @Autowired
    public AdminUsersControllerTests(AdminUsersController adminUsersController, TestUtil testUtil, TestRestTemplate restTemplate) {
    ...
    public void getAllUsers() throws JsonProcessingException {
        ...
        final var response = restTemplate.exchange(testUtil.getEndpoint("/admin/users", port), HttpMethod.GET, entity, String.class);
        final var users = objectMapper.readValue(response.getBody(), User[].class);

        assertThat(users).allMatch(user -> user.getId() != null && user.getFirstName() != null
                && user.getLastName() != null && user.getEmail() != null && user.getDob() != null);
    }
}
